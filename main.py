...
def upload_graph(gml_file):
    ...
    if "all" in gml_file:
        g_all = G.copy()
        return get_elements_from_graph(g_all, mode=True)
    elif "disease" in gml_file:
        g_disease = G.copy()
        return get_elements_from_graph(g_disease, mode=False)
    elif "gene" in gml_file:
        g_gene = G.copy()
        return get_elements_from_graph(g_gene, mode=False)
    else:
        return []


# Da grafo igraph a elements
...
# Divisione in Tab
app.layout = html.Div([
    dcc.Tabs(id="tabs", value='visualizzazione_grafo', children=[
        dcc.Tab(label='Info Principali', value='visualizzazione_grafo'),
    ...
    if tab == 'visualizzazione_grafo':
        return html.Div([
            ...
            dcc.RadioItems(
                id='graph-selector',
                options=[
                    {'label': 'Nessun Grafo', 'value': 'None'},
                    {'label': 'Grafo con tutti i nodi', 'value': 'all'},
                    {'label': 'Grafo con solo malattie', 'value': 'disease'},
                    {'label': 'Grafo con Solo geni', 'value': 'gene'}
                ],
                value='None',
            ...
            html.Div([
                ...
                html.Div([
                    html.H4("Legenda (Clicca per filtrare):"),
        ...
        return html.Div([
            ...
            html.Div([
                html.Div([
                    html.Label('Seleziona metodo di clustering:', style={'margin-right': '10px'}),
                    dcc.Dropdown(
                        ...
                        placeholder="Seleziona metodo di clustering",
                        value='1',
                ...
                html.Div([
                    html.Label('Inserisci un numero positivo:', style={'margin-right': '10px'}),
                    dcc.Input(
                        id='cluster-number',
                        type='number',
                        min=1,
                        step=1,
                        placeholder='Inserisci un numero positivo',
                        style={'width': '150px'}
                    )
                ], id='cluster-number-container'),

                html.Button("Vai", id='apply-clustering', n_clicks=0, disabled=True, style={'margin-top': '10px'})
            ], id='clustering-container'),
            html.Div([
                html.Div(id='clustering-container2', children=[
                    # Qui c'è già la selezione, aggiungi il risultato sotto:
...
def apply_clustering(n_clicks, method, cluster_number):
    if not selected_graph:
        return html.Div("Seleziona prima un grafo nella scheda Visualizzazione Grafo.")

    if selected_graph[0] == "gene":
        return html.Div("Non disponibile")
    ...
    stylesheet = [{
        'selector': 'node',
        'style': {
            'background-color': 'data(fill_color)',  # clustering
            'border-width': 4,
    ...
    elif method == '6':  # Leading Eigenvector
    ...
    if not clustering:
        return html.Div("Errore nell'applicazione del clustering.")
    ...
    # Assortatività media dei cluster
    ...
    g.vs['assortativity cluster'] = [list(assortativity_clusters)[i] for i in g.vs['cluster']]
    ...
    if AVG_ASS:
        AVG_ASS = sum(AVG_ASS) / len(AVG_ASS)
    else:
        AVG_ASS = None
    ...
    # Colora i nodi
    colors = generate_colors(len(list(set(clustering.membership))))
    ...
    if method in cluster_param_methods:
        # Se è un metodo con parametro clusters: grafico metriche
        global results_eb
        global results_ev
        ...
        if method == '1':
            results_eb.append(x)
            results = copy.deepcopy(results_eb)
        else:
            results_ev.append(x)
            results = copy.deepcopy(results_ev)
        ...
        fig.add_trace(go.Scatter(x=df['Clusters'], y=df['Modularity'], mode='lines+markers', name='Modularità'))

        return html.Div([
            html.H4("Partizionamento Visualizzato"),
            cyto.Cytoscape(
            ...
            html.H4("Andamento Metriche"),
            dcc.Graph(figure=fig)
        ])
    else:
        # Metodi senza parametro clusters: tabella metriche
        table = pd.DataFrame([{
            'NMI': NMI,
            'ARI': ARI,
            'NMI LM': NMI_LM,
...
def generate_colors(n):
    ...
    hue = 0  # punto di partenza casuale (puoi randomizzare se vuoi)

    for _ in range(n):
        hue = (hue + golden_ratio_conjugate) % 1  # salta usando il golden ratio
        rgb = colorsys.hsv_to_rgb(hue, 0.8, 0.95)  # leggermente meno saturi e luminosi per visibilità
...
def most_common_in_cluster(g, membership):
    ...
    set_cluster = sorted(list(set(membership)))

    for m in set_cluster:
      b.update({m: np.zeros((1, len(categories)))[0].tolist()})

    for i in range(len(membership)):
      b[membership[i]][categories.index(g.vs["category"][i])] += 1

    new_membership = []

    for m in membership:
      new_membership.append(categories[b[m].index(int(max(b[m])))])
...
# Dato un nodo restituisce le sue informazioni principali
def get_node_info(tapped_node):
    node_info = ""
    if tapped_node:
        node_info = "ATTRIBUTI NODO:\n\n"
